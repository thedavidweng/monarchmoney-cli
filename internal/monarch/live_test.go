...
type mutationShapeType struct {
	Kind   string             `json:"kind"`
	Name   string             `json:"name"`
	OfType *mutationShapeType `json:"ofType"`
}

type mutationShapeField struct {
	Name string `json:"name"`
	Args []struct {
		Name string `json:"name"`
	} `json:"args"`
	Type mutationShapeType `json:"type"`
}

func (p *liveProbe) accountMutationShapes() {
	var introspection struct {
		Schema struct {
			MutationType struct {
				Fields []mutationShapeField `json:"fields"`
			} `json:"mutationType"`
		} `json:"__schema"`
	}
	p.check("accounts/AccountMutationShapes", func() error {
		err := p.svc.Client.Do(p.ctx, &graphql.Request{
			OperationName: "IntrospectAccountMutations",
			Query:         "query IntrospectAccountMutations { __schema { mutationType { fields { name args { name } type { kind name ofType { kind name } } } } } }",
		}, &introspection)
		if err != nil {
			return err
		}
		byName := make(map[string]mutationShapeField, len(introspection.Schema.MutationType.Fields))
		for _, f := range introspection.Schema.MutationType.Fields {
			byName[f.Name] = f
		}
		want := []mutationShapeExpect{
			{"createManualAccount", []string{"input"}, []string{"account", "errors"}},
			{"updateAccount", []string{"input"}, []string{"account", "errors"}},
			{"forceRefreshAccounts", []string{"input"}, []string{"success", "errors"}},
			{"deleteAccount", []string{"id"}, []string{"deleted", "errors"}},
		}
		payloadFields := make(map[string]map[string]bool, len(want))
		for _, w := range want {
			f, ok := byName[w.mutation]
			if !ok {
				continue
			}
			payloadName := f.Type.Name
			if f.Type.Kind == "NON_NULL" && f.Type.OfType != nil {
				payloadName = f.Type.OfType.Name
			}
			var payload struct {
				Type *struct {
					Fields []struct {
			...
			if err := p.svc.Client.Do(p.ctx, &graphql.Request{
				OperationName: "IntrospectMutationPayload",
				Query:         "query IntrospectMutationPayload($name: String!) { __type(name: $name) { fields { name } } }",
				Variables:     map[string]any{"name": payloadName},
			}, &payload); err != nil {
				return err
			}
			if payload.Type == nil {
			...
			present := make(map[string]bool, len(payload.Type.Fields))
			for _, pf := range payload.Type.Fields {
				present[pf.Name] = true
			}
			payloadFields[payloadName] = present
		}
		return verifyMutationShapes(byName, payloadFields, want)
	})
}

type mutationShapeExpect struct {
	mutation string
	args     []string
	payload  []string
}

func payloadTypeName(f *mutationShapeField) string {
	if f.Type.Kind == "NON_NULL" && f.Type.OfType != nil {
...
func shapeField(name string, argNames []string, payload string) mutationShapeField {
	f := mutationShapeField{
		Name: name,
		Type: mutationShapeType{Kind: "NON_NULL", OfType: &mutationShapeType{Kind: "OBJECT", Name: payload}},
	}
	for _, a := range argNames {
		f.Args = append(f.Args, struct {
			Name string `json:"name"`
		}{Name: a})
	}
	return f
...
func shapeExpectations() []mutationShapeExpect {
	return []mutationShapeExpect{
		{"createManualAccount", []string{"input"}, []string{"account", "errors"}},
		{"updateAccount", []string{"input"}, []string{"account", "errors"}},
		{"forceRefreshAccounts", []string{"input"}, []string{"success", "errors"}},
		{"deleteAccount", []string{"id"}, []string{"deleted", "errors"}},
...
func shapeFixtures() (byName map[string]mutationShapeField, payloads map[string]map[string]bool) {
	byName = map[string]mutationShapeField{
		"createManualAccount":  shapeField("createManualAccount", []string{"input"}, "CreateManualAccountPayload"),
		"updateAccount":        shapeField("updateAccount", []string{"input"}, "UpdateAccountPayload"),
		"forceRefreshAccounts": shapeField("forceRefreshAccounts", []string{"input"}, "ForceRefreshAccountsPayload"),
		"deleteAccount":        shapeField("deleteAccount", []string{"id"}, "DeleteAccountPayload"),
	}
	payloads = map[string]map[string]bool{
		"CreateManualAccountPayload":  {"account": true, "errors": true},
		"UpdateAccountPayload":        {"account": true, "errors": true},
		"ForceRefreshAccountsPayload": {"success": true, "errors": true},
		"DeleteAccountPayload":        {"deleted": true, "errors": true},
	}
	return byName, payloads
}

func TestVerifyMutationShapes(t *testing.T) {
	t.Run("current shapes pass", func(t *testing.T) {
		byName, payloads := shapeFixtures()
		if err := verifyMutationShapes(byName, payloads, shapeExpectations()); err != nil {
			t.Fatalf("verifyMutationShapes() error = %v", err)
		}
	})

	t.Run("positional args detected", func(t *testing.T) {
		byName, payloads := shapeFixtures()
		byName["createManualAccount"] = shapeField("createManualAccount", []string{"name", "type", "balance"}, "CreateManualAccountPayload")
		err := verifyMutationShapes(byName, payloads, shapeExpectations())
		if err == nil || !strings.Contains(err.Error(), "createManualAccount") {
			t.Fatalf("verifyMutationShapes() error = %v, want createManualAccount args drift", err)
		}
	})

	t.Run("removed mutation detected", func(t *testing.T) {
		byName, payloads := shapeFixtures()
		delete(byName, "forceRefreshAccounts")
		err := verifyMutationShapes(byName, payloads, shapeExpectations())
		if err == nil || !strings.Contains(err.Error(), "forceRefreshAccounts") {
			t.Fatalf("verifyMutationShapes() error = %v, want forceRefreshAccounts missing", err)
		}
	})

	t.Run("renamed payload field detected", func(t *testing.T) {
		byName, payloads := shapeFixtures()
		payloads["DeleteAccountPayload"] = map[string]bool{"ok": true}
		err := verifyMutationShapes(byName, payloads, shapeExpectations())
		if err == nil || !strings.Contains(err.Error(), "deleted") {
			t.Fatalf("verifyMutationShapes() error = %v, want deleted field drift", err)
		}
	})

	t.Run("unknown payload type detected", func(t *testing.T) {
		byName, payloads := shapeFixtures()
		delete(payloads, "UpdateAccountPayload")
		err := verifyMutationShapes(byName, payloads, shapeExpectations())
		if err == nil || !strings.Contains(err.Error(), "updateAccount") {
			t.Fatalf("verifyMutationShapes() error = %v, want updateAccount payload drift", err)
		}
	})
}

func verifyMutationShapes(byName map[string]mutationShapeField, payloadFields map[string]map[string]bool, want []mutationShapeExpect) error {
	for _, w := range want {
		f, ok := byName[w.mutation]
		if !ok {
			return fmt.Errorf("mutation %s missing from live schema: upstream shape changed", w.mutation)
		}
		gotArgs := make([]string, 0, len(f.Args))
		for _, a := range f.Args {
			gotArgs = append(gotArgs, a.Name)
		}
		slices.Sort(gotArgs)
		if !slices.Equal(gotArgs, slices.Sorted(slices.Values(w.args))) {
			return fmt.Errorf("mutation %s args = %v, want %v: upstream shape changed", w.mutation, gotArgs, w.args)
		}
		present, ok := payloadFields[payloadTypeName(&f)]
		if !ok {
			return fmt.Errorf("mutation %s payload missing from live schema: upstream shape changed", w.mutation)
		}
		var missing []string
		for _, name := range w.payload {
			if !present[name] {
				missing = append(missing, name)
			}
		}
		if len(missing) > 0 {
			return fmt.Errorf("mutation %s payload missing fields %v: upstream shape changed", w.mutation, missing)
		}
	}
	return nil
}
